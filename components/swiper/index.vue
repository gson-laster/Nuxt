...
export default {
  ...
  mounted() {
    ...
    for (let i = 0; i < items.length; i++) {
      if (hasClass(items[i], 'banner')) {
        ItemHeightArr.push(items[i].offsetHeight);
      }
    }

    if (hasClass(element, 'banner')) {
      let warrpHeight = Math.max.apply(null, ItemHeightArr)
      element.style.height = warrpHeight + 'px'
    }
  }
};
</script>
