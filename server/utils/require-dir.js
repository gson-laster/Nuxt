...
const tree = {}
const notTree = {}
const dirFilter = absolutePathStr => fs.lstatSync( absolutePathStr )
  .isDirectory()
const jsFileFilter = absolutePathStr => fs.lstatSync( absolutePathStr )
  .isFile() && absolutePathStr.endsWith( '.js' )
const openDir = dir => {
  let readRes = fs.readdirSync( dir )
  dirPath.push( ...readRes.map( p => path.join( dir, p ) )
    .filter( jsFileFilter ) )
  readRes.map( p => path.join( dir, p ) )
    .filter( dirFilter )
    .forEach( i => openDir( i ) )
}
const camelCase = name => {
  let spil = name.trim()
    .split( /[^0-9a-zA-Z]/ )
    .filter( i => ( i !== '' ) )
  return ( spil.shift() + spil.map( i => ( `${i.charAt(0).toUpperCase()}${i.substr(1)}` ) )
    .join( '' ) )
}
...
module.exports = ( thisDir, dir, options ) => {
  ...
  dirPath.forEach( i => {
    // do tree
    propsArrHanler( tree[ baseDir ], i.split( baseDir )[ 1 ].split( path.sep )
      .slice( 0, -1 )
      .filter( i => ( i !== '' ) ) )[ camelCase( path.basename( i, 'js' ) ) ] = require( i )
    // do notTree
    notTree[ baseDir ][ options.nameHandler( camelCase( i.split( baseDir )[ 1 ].split( path.sep )
      .slice( 0, -1 )
      .filter( i => ( i !== '' ) )
      .concat( path.basename( i, 'js' ) )
      .map( i => camelCase( i ) )
      .join( '.' ) ) ) ] = require( i )
  } )
